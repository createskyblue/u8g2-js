...
（字体源：**SimHei 黑体** `C:\Windows\Fonts\simhei.ttf`），字符集覆盖：
...
改用 SimHei 后，**CJK 字形统一坐基线**（底部对齐基线、顶部一致），不再有 MapleMono
...
u8g2-js/
  ...
  test/test.js        # Node 无头测试套件（29 项）
```

## 验证

1. **与原版 C 库逐字节交叉验证**（`tools/cverify/build.sh`，需 gcc）：
   编译真 U8G2 C 库渲染 15 个场景（文本/图形/旋转/裁剪/XOR/按钮/Unicode/**全量中文字库渲染**），
   与 JS 移植逐字节比对 —— **全部一致**（含字体数据与 C 数组逐字节一致）。
2. **Node 无头测试**：`node --test test/`，29 项全过。
